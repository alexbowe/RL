...
def init_ray(log_dir: Optional[str] = None) -> None:
    ...
    disable_auto_attach = os.environ.get(
        "NEMO_RL_DISABLE_RAY_AUTO_ATTACH", ""
    ).lower() in {"1", "true", "yes", "on"}

    if disable_auto_attach:
        logger.info("Skipping Ray auto-attach; starting a fresh local cluster.")
    else:
        ...
        try:
            ...
            logger.debug("No reusable Ray cluster found, will start a new one.")
            ray.shutdown()
