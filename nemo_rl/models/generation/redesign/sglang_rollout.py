...
async def generate_rollout_async_stream(
    args: Namespace, rollout_id: int, data_source: Callable[[int], list[list[Sample]]]
):
    """Streaming version of generate_rollout_async that yields each completed group immediately.

    Yields:
        list[Sample]: a group of samples, yielded as soon as the group finishes generation and RM.
    """
    assert args.rollout_global_dataset
    ...
    # Submit all tasks upfront so we can use as_completed
    while state.remaining_batch_size < target_data_size:
        samples = data_source(args.over_sampling_batch_size)
        state.submit_generate_tasks(samples)

    # Snapshot the pending tasks and iterate by completion order
    all_tasks = list(state.pendings)
    state.pendings = set()

    data = []
    all_data = []
    do_print = True
    pbar = tqdm(total=target_data_size * args.n_samples_per_prompt, desc="Rollout generation (stream)")

    for coro in asyncio.as_completed(all_tasks):
        group: list[Sample] = await coro
    ...
    state.pendings = {t for t in all_tasks if not t.done()}
